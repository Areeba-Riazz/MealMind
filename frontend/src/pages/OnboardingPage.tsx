...
import { getOnboardingCompleted, setOnboardingCompleted } from '../lib/firestoreUserData';
...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const done = await getOnboardingCompleted(user.uid);
  ...
  const finish = async () => {
    if (user?.uid) await setOnboardingCompleted(user.uid, true);
    navigate('/dashboard', { replace: true });
  };
  ...
  if (checking) {
    return (
      <>
        <style>{`
          .ob-full { min-height: 100vh; background: #0c0c0c; display: flex; align-items: center; justify-content: center; color: rgba(255,255,255,0.35); font: 0.9rem 'DM Sans', sans-serif; }
        `}</style>
        <div className="ob-full">Loading…</div>
      </>
    );
  }
  ...
  return (
    <>
      <style>{`
        ...
        .ob-field input { width:100%; padding:0.85rem 1rem; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.1); border-radius:13px; color:#f2ede4; font:0.92rem 'DM Sans',sans-serif; outline:none; transition:border-color 0.2s; box-sizing:border-box; }
        ...
        <div className="ob-wrap">
          <div className="ob-steps" role="list">
            {STEPS.map((s) => (
              <div
                key={s.id}
                className={`ob-step${s.done ? ' done' : s.current ? ' current' : ''}`}
                role="listitem"
              >
          ...
          <div className="ob-card">
              ...
              <div className="ob-skill-pills">
                {[
                  ...
                  <button
                    key={s.val}
                    type="button"
                    className={`ob-pill${skill === s.val ? ' on' : ''}`}
                    onClick={() => setSkill(s.val)}
                  >
                    {s.label}
                  </button>
                ))}
            ...
            <div className="ob-actions">
              <button type="button" className="ob-btn-ghost" disabled>
                ← Back
              </button>
              <button type="button" className="ob-btn-next" onClick={() => void finish()}>
                Next →
              </button>
              <button type="button" className="ob-btn-skip" onClick={() => void finish()}>
                Skip for now
              </button>
            </div>
          </div>
